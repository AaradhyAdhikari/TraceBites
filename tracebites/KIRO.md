# Handoff: get TraceBites onto GitHub and running
...
Create an **empty** repository at `github.com/AaradhyAdhikari/tracebites` — no
README, no .gitignore, no licence, because any of those create a commit that
makes the first push non-fast-forward. Then:

```bash
git remote add origin https://github.com/AaradhyAdhikari/tracebites.git
git push -u origin main
```

If `gh` is authenticated, `gh repo create AaradhyAdhikari/tracebites --public --source=. --remote=origin --push` does both steps.
...
```bash
npm run verify             # expect: PASS — 28 passed, 0 failed
```

These are the only parts of the repo that have been executed and are known green:
the hash chain and batch codes (28 assertions). They assert that key order does not change a digest,
...
5. Corrupt one event and confirm the page notices:
   ```sql
   -- in psql: docker exec -it tracebites-db psql -U tracebites
...
chain adapter interface · `contracts/src/TraceBitesRegistry.sol`.
